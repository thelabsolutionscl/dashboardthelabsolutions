...
async function request(path,options={}){const b=base(),t=token();if(!b||!t)throw new Error('controller/sesión no disponible');const opts={cache:'no-store',signal:AbortSignal.timeout(options.timeout||8000),...options};delete opts.timeout;return readJson(await fetch(url(path),opts));}
...
function install(root){if(installed||!root)return false;target=root;installed=true;setTimeout(()=>refresh(true),1200);if(typeof root.setInterval==='function')root.setInterval(()=>{if(!root.document?.hidden)refresh(false);},30000);if(typeof root.addEventListener==='function')root.addEventListener('focus',()=>refresh(true));return true;}
...
(function _loadDashboardUiExtensions(){
  ...
  load('js/machineops-farm-queue-adapter.js','cola única MachineOps/Farm Controller');
})();
