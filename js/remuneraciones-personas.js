 ...
 * Permite registrar jornadas con fecha, fracción de día y detalle.
...
function remPersonaMonthState(p,mk=remPersonaMonthKey()){
  ...
  const dias=remPersonaClamp(ajusteDias+detallados,0,31),rate=Math.max(0,Math.round(remPersonaNumber(e.valorDia))),base=Math.max(0,Math.round(remPersonaNumber(s[name])));
  return{base,rate,ajusteDias,jornadas,dias,pay:Math.round(rate*dias)};
}
...
function remPersonaRenderDraft(){
  ...
  const list=document.getElementById('remPersonaJornadasList'),summary=document.getElementById('remPersonaPreview');
  const days=remPersonaDraftDays(),pay=Math.round(Math.max(0,remPersonaNumber(document.getElementById('remPersonaRate')?.value))*days);
  if(summary)summary.innerHTML='<b>'+String(days).replace('.',',')+' días</b> · Pago por días: <b>'+remPersonaMoney(pay)+'</b>';
  if(!list)return;
...
function remPersonaOpenModal(id){
  ...
  remPersonaDraft={personId:remPersonaPersonKey(p),personName:remPersonaPersonName(p),month:mk,base:r.base,rate:r.rate,ajusteDias:r.ajusteDias,jornadas:r.jornadas.map(j=>({...j}))};
...
function remPersonaUpdateButton(personId,days){
  const buttons=Array.from(document.querySelectorAll('[data-rem-person-btn]'));const b=buttons.find(x=>String(x.dataset.remPersonBtn)===String(personId));if(!b)return;const label=days>0?'💰 '+String(days).replace('.',',')+' días':'💰 Remuneración';if(b.textContent!==label)b.textContent=label;
}
function remPersonaSaveModal(e){
  ...
  try{
    ...
    const base=Math.max(0,Math.round(remPersonaNumber(document.getElementById('remPersonaBase')?.value))),rate=Math.max(0,Math.round(remPersonaNumber(document.getElementById('remPersonaRate')?.value))),ajuste=remPersonaClamp(document.getElementById('remPersonaAjuste')?.value,0,31),mk=remPersonaDraft.month;
    ...
    const prev=entry.meses[mk]&&typeof entry.meses[mk]==='object'?entry.meses[mk]:{};
    entry.meses[mk]={...prev,dias,ajusteDias:ajuste,jornadas,actualizadoEn:new Date().toISOString()};
    ...
    remPersonaUpdateButton(pk,dias);
    const modal=document.getElementById('remPersonaModal');if(modal)modal.style.display='none';
    remPersonaDraft=null;
    try{root.dispatchEvent?.(new CustomEvent('remuneraciones-dias-updated',{detail:{personId:pk,month:mk,dias}}));}catch(_){}
    remPersonaToast('✓ Remuneración de '+name+' guardada','success');
  }catch(err){
...
function remPersonaInjectButtons(){
  ...
  Array.from(body.querySelectorAll('tr')).forEach((tr,i)=>{
    ...
    const pk=remPersonaPersonKey(p),r=remPersonaMonthState(p),label=r.dias>0?'💰 '+String(r.dias).replace('.',',')+' días':'💰 Remuneración';
