 ...
 * Observa Moonraker y registra el evento terminal en /farm/production/events,
 * incluso cuando no hay ningún dashboard abierto.
 */
...
function terminalDecision(job,live,previous={}){
  const state=String(live?.state||'').toLowerCase(),file=basename(live?.filename),expected=basename(job?.filename),matches=!file||!expected||file===expected;
  const active=['printing','paused'].includes(state);if(active&&matches)return{terminal:false,seenActive:true};
  if(!matches)return{terminal:false,seenActive:!!previous.seenActive};
  if(state==='complete')return{terminal:true,result:'Completado',seenActive:true};
  if(state==='cancelled'||state==='canceled')return{terminal:true,result:'Cancelado',seenActive:true};
  if(['error','shutdown'].includes(state))return{terminal:true,result:'Fallido',seenActive:true};
  return{terminal:false,seenActive:!!previous.seenActive};
}
async function moonraker(ip){
  const r=await fetch(`http://${ip}:7125/printer/objects/query?print_stats&webhooks`,{cache:'no-store',signal:AbortSignal.timeout(6000)});if(!r.ok)throw new Error('Moonraker '+r.status);const d=await r.json();const s=d?.result?.status||{},p=s.print_stats||{},w=s.webhooks||{};return{state:String(p.state||w.state||'').toLowerCase(),filename:p.filename||'',message:p.message||w.state_message||''};
}
function eventFor(job,result,end=Date.now()){
  const start=Date.parse(job?.startedAt||0)||Math.max(0,end-Number(job?.secs||0)*1000),dur=Math.max(0,(end-start)/60000);
  return{eventId:'farm-'+crypto.createHash('sha256').update(String(job.id)).digest('hex').slice(0,32),machineId:job.machineId,id:job.machineId,nombre:job.machineName||'',file:job.filename||'',start,end,dur,result,filamentMm:0,ts:end,queueJobId:job.id,machineOpsJobId:job.machineOpsJobId||''};
}
...
async function scanOnce(){
  ...
  try{
    const queue=readJson(QUEUE_FILE,{jobs:[]}),registry=readJson(REGISTRY_FILE,{machines:[]}),saved=readJson(STATE_FILE,{version:1,jobs:{}});saved.jobs=saved.jobs||{};
    const byId=new Map((registry.machines||[]).map(m=>[m.id,m]));let dirty=false;
    ...
    // Mantener marcadores terminales un año; evita que lifecycle.json crezca sin límite.
...
module.exports={terminalDecision,eventFor,atomicWrite,scanOnce,status:()=>({running,lastRun,lastError})};
