 ...
 *
 * Se carga con:
 *   node -r ./farm-production-preload.js ./farm-controller.js
 *
 * Intercepta sólo /farm/production* antes del handler principal. El resto de
 * las rutas sigue exactamente en farm-controller.js.
 */
...
function normalizeOdometerEntry(raw) {
  const v = raw && typeof raw === 'object' ? raw : {};
  return {
    hours: nonNegative(v.hours),
    filamentMm: nonNegative(v.filamentMm),
    prints: Math.floor(nonNegative(v.prints)),
    failures: Math.floor(nonNegative(v.failures)),
  };
}
function eventKey(raw) {
  ...
  const basis = [machineId, file, start, end, result].join('\0');
...
function normalizeEvent(raw) {
  ...
  const result = String(raw.result || '').trim() || 'Desconocido';
  const event = {
    eventId: String(raw.eventId || eventKey({ ...raw, machineId })).slice(0, 80),
    machineId,
    id: machineId,
    nombre: String(raw.nombre || '').slice(0, 120),
    numG: raw.numG == null ? '' : String(raw.numG).slice(0, 32),
    file: String(raw.file || '').slice(0, 240),
    start,
    end,
    dur,
    result: result.slice(0, 40),
    filamentMm: nonNegative(raw.filamentMm),
    ts: finite(raw.ts, end || Date.now()),
  };
  return event;
}
function mergeHistory(existing, incoming, limit = HISTORY_LIMIT) {
  const map = new Map();
  for (const raw of [...(Array.isArray(existing) ? existing : []), ...(Array.isArray(incoming) ? incoming : [])]) {
    const e = normalizeEvent(raw);
    if (!e) continue;
    const prev = map.get(e.eventId);
    if (!prev || e.ts >= prev.ts) map.set(e.eventId, e);
  }
  return [...map.values()].sort((a, b) => (b.ts || b.end || 0) - (a.ts || a.end || 0)).slice(0, limit);
}
function deriveOdometer(history) {
  ...
  for (const raw of Array.isArray(history) ? history : []) {
    ...
    const completed = e.result.toLowerCase() === 'completado';
    if (completed) {
      row.hours += e.dur / 60;
      row.prints += 1;
    } else {
...
function normalizeProduction(raw) {
  ...
  const odometer = {};
  for (const [machineId, value] of Object.entries(p.odometer && typeof p.odometer === 'object' ? p.odometer : {})) {
    if (!machineId) continue;
    odometer[machineId] = normalizeOdometerEntry(value);
  }
  return {
    version: 1,
    updatedAt: nonNegative(p.updatedAt),
    history: mergeHistory([], p.history),
    odometer,
  };
}
function mergeMigration(state, payload, now = Date.now()) {
  const target = normalizeProduction(state);
  const incomingHistory = mergeHistory([], payload?.history);
  target.history = mergeHistory(target.history, incomingHistory);
  const derived = deriveOdometer(incomingHistory);
  const incomingOdo = payload?.odometer && typeof payload.odometer === 'object' ? payload.odometer : {};
  const ids = new Set([...Object.keys(target.odometer), ...Object.keys(derived), ...Object.keys(incomingOdo)]);
  for (const id of ids) {
    const current = normalizeOdometerEntry(target.odometer[id]);
    const fromHistory = normalizeOdometerEntry(derived[id]);
    const fromBrowser = normalizeOdometerEntry(incomingOdo[id]);
    target.odometer[id] = {
      hours: Math.max(current.hours, fromHistory.hours, fromBrowser.hours),
      filamentMm: Math.max(current.filamentMm, fromHistory.filamentMm, fromBrowser.filamentMm),
      prints: Math.max(current.prints, fromHistory.prints, fromBrowser.prints),
      failures: Math.max(current.failures, fromHistory.failures, fromBrowser.failures),
    };
  }
...
function recordEvent(state, raw, now = Date.now()) {
  ...
  if (!event) return { state: target, event: null, added: false };
  if (target.history.some(h => h.eventId === event.eventId)) return { state: target, event, added: false };
  target.history = mergeHistory([event], target.history);
  const row = target.odometer[event.machineId] || (target.odometer[event.machineId] = normalizeOdometerEntry(null));
  if (event.result.toLowerCase() === 'completado') {
    row.hours += event.dur / 60;
    row.prints += 1;
  } else {
    row.failures += 1;
  }
  row.filamentMm += event.filamentMm;
  target.updatedAt = now;
  return { state: target, event, added: true };
}

let production = normalizeProduction(readJson(PRODUCTION_FILE, null));
let writeChain = Promise.resolve();
function persistProduction() {
  production.updatedAt = Date.now();
  writeChain = writeChain.then(() => atomicWrite(PRODUCTION_FILE, production))
    .catch(e => console.error('[production] persist', e));
  return writeChain;
}
function snapshot() {
  return {
    ...
    odometer: production.odometer,
    historyCount: production.history.length,
...
async function handleProduction(req, res) {
  ...
  if (p === '/farm/production/events' && req.method === 'POST') {
    ...
    try {
      ...
      const out = recordEvent(production, body);
      production = out.state;
      if (out.added) await persistProduction();
      return json(res, 200, { ok: true, added: out.added, event: out.event, production: snapshot() }, { 'X-Farm-Role': role });
    } catch (e) {
      return json(res, 400, { ok: false, error: e.message });
    }
  }
...
function installPreload() {
  if (http.__TLS_FARM_PRODUCTION_PRELOAD__) return false;
  http.__TLS_FARM_PRODUCTION_PRELOAD__ = true;
  ...
  http.createServer = function patchedCreateServer(options, requestListener) {
    ...
    const wrapped = function(req, res) {
      let pathname = '';
      try { pathname = new URL(req.url, 'http://farm.local').pathname; } catch (_) {}
      if (pathname === '/farm/production' || pathname.startsWith('/farm/production/')) {
        handleProduction(req, res).catch(e => {
...
module.exports = {
  eventKey, normalizeEvent, mergeHistory, deriveOdometer, normalizeProduction,
  mergeMigration, recordEvent, roleForToken, installPreload, snapshot,
};
