...
test('operator importa G-code directamente desde Moonraker y repetir ID es idempotente',async()=>{const payload={id:'mops-job-1',machineId:'m1',sourceFile:'jobs/pieza.gcode',secs:120,grams:20,priority:50};let r=await fetch(`http://127.0.0.1:${PORT}/farm/queue/import`,{method:'POST',headers:{'Content-Type':'application/json','X-Bridge-Token':'op'},body:JSON.stringify(payload)}),d=await r.json();assert.equal(r.status,201,d.error||JSON.stringify(d));assert.equal(d.ok,true);assert.equal(d.job.id,'mops-job-1');assert.equal(d.job.filename,'mops-job-1--pieza.gcode');assert.equal(d.bytes,Buffer.byteLength(GCODE));r=await fetch(`http://127.0.0.1:${PORT}/farm/queue/import`,{method:'POST',headers:{'Content-Type':'application/json','X-Bridge-Token':'op'},body:JSON.stringify(payload)});d=await r.json();assert.equal(r.status,200);assert.equal(d.idempotent,true);assert.equal(JSON.parse(fs.readFileSync(process.env.FARM_QUEUE_FILE,'utf8')).jobs.length,1);});
